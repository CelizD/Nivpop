...
function exportCSV(results: NivResult[]) {
  const hdr = ["ID","Fecha","Hora","Modo","FlavorId","Sabor","Nombre1","Nombre2","Compat"];
  const rows = results.map((r) => [r.id,r.fecha,r.timeStr,r.modo,r.flavorId,r.sabor,r.nombre1,r.nombre2??"",r.compat??""]);
          ...
          <div className="space-y-8">
            ...
            {activeHours.length > 0 && (
              <div>
                ...
                <div className="flex items-end gap-1 h-16">
                  {Array.from({length:24},(_,h)=>h).filter(h=>hourly[h]||activeHours.includes(h)).map((h) => (
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <p className="text-[9px] tracking-[0.4em] text-muted/50 uppercase">ÚLTIMAS {Math.min(displayed.length, 50)}</p>
                {displayed.length > 0 && (
                  ...
                  <div className="space-y-2">
                    {displayed.slice(0,50).map((r) => (
                      <div key={r.id} className="flex items-center gap-3 border border-paper/8 px-4 py-3">
                        ...
                        <p className="text-[10px] text-muted/35 flex-shrink-0">{r.timeStr}</p>
                      </div>
                    ))}
                  </div>
                )
        ...
        {tab === "sabores" && (
          <div className="space-y-10">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted/35 mb-5">Los no disponibles aparecen como &quot;Agotado&quot; en el catálogo.</p>
              <div className="space-y-2">
                {ALL_IDS.map((id) => {
